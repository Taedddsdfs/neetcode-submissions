...
class NumMatrix:
    ...
    def sumRegion(self, row1: int, col1: int, row2: int, col2: int) -> int:
        ...
        return (
        self.pref[row2 + 1][col2 + 1]  # 1. Total (A + B + C + Target)
        - self.pref[row1][col2 + 1]    # 2. Top 빼기 (A + B)
        - self.pref[row2 + 1][col1]    # 3. Left 빼기 (A + C)
        + self.pref[row1][col1]        # 4. 중복으로 빠진 대각선 구석탱이 복구 (A)
        )
